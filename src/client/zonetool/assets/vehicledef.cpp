...
#define VEHICLE_READ_VALUE_ARR(__type__, __field__, __count__) \
	...
	void IVehicleDef::add_script_string(scr_string_t* ptr, std::string str)
	{
	...
	std::string IVehicleDef::get_script_string(scr_string_t* ptr)
	{
		for (std::uint32_t i = 0; i < this->script_strings.size(); i++)
		{
			if (this->script_strings[i].first == ptr)
	...
	VehicleDef* IVehicleDef::parse(const std::string& name, ZoneMemory* mem)
	{
		...
		for (auto i = 0; i < 4; i++)
		{
			...
			{
				this->add_script_string(&vehicle->trophyTags[i], data["trophyTags"][i].get<std::string>());
			}
		}
		...
		{
			this->add_script_string(&vehicle->audioOriginTag, data["audioOriginTag"].get<std::string>());
		}
		...
		{
			this->add_script_string(&vehicle->audioOriginTagAlt, data["audioOriginTagAlt"].get<std::string>());
		}
